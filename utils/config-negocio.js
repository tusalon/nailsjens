...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '52641dc0-619f-45b4-bb23-b167eeedf518'; // ID de Nails-Jens
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Nails-Jens';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '59302541';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'rodriguezsalgadojennifer91@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Nails-Jens!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'nails-jens';
};
...
console.log('✅ config-negocio.js listo para Nails-Jens');
